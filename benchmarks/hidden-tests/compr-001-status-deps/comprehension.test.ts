...
export const expectedAnswers = {
  ...
  q2: {
    ...
    changes: [
      ...
      { file: 'src/validation.ts', change: 'Add "review" to list of valid status values if a validator exists' },
      { file: 'tests/store.test.ts', change: 'Tests may need updating for countsByStatus expectations' },
    ],
  },
  q3: {
    question: 'Relationship between validation.ts and cli.ts?',
    answer: 'validation.ts provides reusable validators (validateCreateTask, validateTaskId). cli.ts calls store.create() and store.update() — store.create() internally calls validateCreateTask() from validation.ts. The CLI itself should delegate to validation.ts for update field validation rather than doing inline checks.',
  },
  q4: {
    question: 'Is TaskStore coupled to CLI?',
    answer: 'No. TaskStore is a standalone class with no CLI dependencies. It accepts typed inputs and returns typed outputs. The CLI (TaskCli) depends on TaskStore, not the reverse. TaskStore can be used programmatically without the CLI.',
  },
};
